...
public abstract class Expression {
    ...
    public static void main(String[] args) throws ExceptionInvalidExpression {
        Expression e = create("hola & adeu | patata");
    }
    ...
    public static Expression create1(String exp) throws ExceptionInvalidExpression {
        ...
        int n = exp.length();
        List<Character> act = new ArrayList<>();
        for (int i = 0; i < n; ++i) {
            char c = exp.charAt(i);
            act.add(c);
            if (tocaOp && c != ' ') {
                if (c == '&') if (firstAnd == -1) firstAnd = i;
                else if (c == '|') if (firstOr == -1) firstOr = i;
            ...
            if (c == '(') {
                ...
                if (c == ' ') act.remove(act.size() - 1);
                list.add(String.valueOf(act));
                act = new ArrayList<>();;
                tocaOp = !tocaOp;
            }
        }
    ...
    public static Expression create2(List<String> list) throws ExceptionInvalidExpression {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            else if (list.get(i) == "&" && firstAnd == -1) firstAnd = -1;
        }
    ...
    public static Expression create3(String exp) throws ExceptionInvalidExpression {
        switch (exp.charAt(0)) {
            ...
            case '(':
            case '"':
                if (exp.charAt(exp.length()-1) != exp.charAt(0)) throw new ExceptionInvalidExpression(exp);
